...
export const sendPushNotification = functions.firestore
  ...
  .onCreate(async (snap, context) => {
    ...
    try {
      ...
      if (!userData) {
        console.log("❌ Pas de données utilisateur:", notification.userId);
        return null;
      }
 ...
 */
export const cleanInvalidTokens = functions.firestore
  .document("notifications/{notificationId}")
  .onUpdate(async (change, context) => {
    ...
    if (
      newData.pushErrorCode === "messaging/invalid-registration-token" ||
...
export const sendTestNotification = functions.https.onCall(
  async (data, context) => {
    ...
    const message =
      data.message ||
      "Ceci est une notification de test avec son et vibration!";
